 ...
 */
const formatValue = (value: number, format: ValueFormat, locale: string): string => {
  switch (format) {
    case 'currency':
...
export const KPICard: React.FC<KPICardProps> = ({
  ...
  const colorConfig: Record<KPIColor, { border: string; iconBg: string; icon: string }> = {
    blue: {
    ...
    yellow: {
      border: 'border-gray-200 dark:border-gray-700 hover:border-yellow-300 dark:hover:border-yellow-600',
  ...
  const getTrendIcon = (direction: TrendDirection): React.ReactNode => {
    ...
    switch (direction) {
      case 'up':
        return <TrendingUp className={`${iconClass} text-green-500`} aria-hidden="true" />;
      case 'down':
        return <TrendingDown className={`${iconClass} text-red-500`} aria-hidden="true" />;
      default:
        return <Minus className={`${iconClass} text-gray-500`} aria-hidden="true" />;
    }
  };
  ...
  if (loading) {
    return (
      <div className={`${config.padding} rounded-xl border-2 ${colors.border} bg-white dark:bg-gray-800 ${className}`}>
        <div className="animate-pulse">
          <div className="flex items-center gap-3 mb-3">
            <div className={`${config.iconPadding} rounded-lg ${colors.iconBg}`}>
              <div className={`${config.iconSize} bg-gray-300 dark:bg-gray-600 rounded`} />
            </div>
          ...
          {description && <div className="h-4 bg-gray-300 dark:bg-gray-600 rounded w-full mb-3" />}
        </div>
      </div>
    );
  }
      ...
      <div className="flex items-start justify-between">
        <div className="flex-1">
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <h3 className={`${config.titleSize} font-semibold text-gray-700 dark:text-gray-300`}>
              {title}
            </h3>
          </div>
          ...
          {/* Value */}
          <div className="mb-3">
            <p className={`${config.valueSize} font-bold text-gray-900 dark:text-white`}>
              {formattedValue}
            </p>
          </div>
          ...
          {/* Description */}
          {description && (
            <p className={`${config.descriptionSize} text-gray-600 dark:text-gray-400 mb-3`}>
              {description}
            </p>
          )}
          ...
          {trend && (
            <div className="flex items-center gap-2">
              ...
              <span className={`${config.trendSize} font-medium ${getTrendColor(trend.direction)}`}>
