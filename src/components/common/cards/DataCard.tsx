...
export interface DataCardProps {
  ...
  readonly headerColor?: 'blue' | 'green' | 'red' | 'yellow' | 'purple' | 'gray';
          ...
          return (
            <div key={field.id} className="flex items-start justify-between gap-2">
              <div className="flex items-start gap-2 flex-1 min-w-0">
                {Icon && <Icon className="h-4 w-4 text-gray-400 dark:text-gray-600 mt-0.5 flex-shrink-0" />}
                <div className="min-w-0">
                  <p className="text-sm font-medium text-gray-600 dark:text-gray-400">{field.label}</p>
                  <p className={`${variantClasses[field.variant || 'default']} break-words`}>
                    {field.value}
                  </p>
                </div>
              </div>
