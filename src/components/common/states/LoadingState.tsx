...
const SkeletonLoading: React.FC<{ size: LoadingSize }> = ({ size }) => (
  <div className="space-y-3 w-full max-w-md">
    <div className={`${sizeConfigs.skeleton[size]} bg-gray-200 dark:bg-gray-700 rounded-lg animate-pulse`} />
...
const DotsLoading: React.FC<{ size: LoadingSize }> = ({ size }) => {
  ...
  return (
    <div className="flex items-center space-x-2" role="status" aria-label="Loading">
      <div className={`${dotClass} bg-blue-600 rounded-full animate-bounce`} style={{ animationDelay: '0ms' }} />
      <div className={`${dotClass} bg-blue-600 rounded-full animate-bounce`} style={{ animationDelay: '150ms' }} />
      <div className={`${dotClass} bg-blue-600 rounded-full animate-bounce`} style={{ animationDelay: '300ms' }} />
    </div>
  );
};
