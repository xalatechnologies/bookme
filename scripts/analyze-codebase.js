...
function analyzeFile(filePath) {
  ...
  const analysis = {
    ...
    localization: {
      ...
      hasTranslationHook: /const\s+{\s*t\s*}\s*=\s*useTranslation/.test(
        content
      ),
...
function getFileType(relativePath, content) {
  ...
  if (relativePath.startsWith("components/features/"))
...
const inventory = {
  ...
  localization: {
    withTranslationHook: analyses.filter(
      (a) => a.localization.hasTranslationHook
    ).length,
    withHardcodedStrings: analyses.filter(
      (a) => a.localization.hardcodedStrings.length > 0
    ).length,
    totalHardcodedStrings: analyses.reduce(
      (sum, a) => sum + a.localization.hardcodedStrings.length,
      0
    ),
  },
  typescript: {
    filesWithExplicitAny: analyses.filter(
      (a) => a.typescript.hasExplicitAny > 0
    ).length,
    totalExplicitAny: analyses.reduce(
      (sum, a) => sum + a.typescript.hasExplicitAny,
      0
    ),
    filesWithUnusedImports: analyses.filter(
      (a) => a.typescript.unusedImports.length > 0
    ).length,
  },
  components: analyses
    .filter(
...
const tsIssues = analyses
  .filter(
    (a) =>
      a.typescript.hasExplicitAny > 0 || a.typescript.unusedImports.length > 0
  )
  .map((a) => ({
    file: a.path,
...
console.log(
  `  With translation hook: ${inventory.localization.withTranslationHook}`
);
console.log(
  `  With hardcoded strings: ${inventory.localization.withHardcodedStrings}`
);
console.log(
  `  Total hardcoded strings: ${inventory.localization.totalHardcodedStrings}`
);
console.log(
  `  Files with explicit 'any': ${inventory.typescript.filesWithExplicitAny}`
);
console.log(`  Total 'any' usages: ${inventory.typescript.totalExplicitAny}`);
console.log(
  `  Files with unused imports: ${inventory.typescript.filesWithUnusedImports}`
);
