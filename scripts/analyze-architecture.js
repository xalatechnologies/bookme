...
function analyzeAuth(content, filePath) {
  ...
  return {
    path: relativePath,
    hasAuthGuard: /ProtectedRoute|RequireRole|PermissionGuard|RoleGuard/.test(content),
...
function extractPermissionReferences(content) {
  ...
  const permPattern = /['"](?:bookings|facilities|users|reports)\.(?:read|write|delete|approve)['"]/gi;
...
function analyzeStyling(content, filePath) {
  ...
  return {
    path: relativePath,
    styling: {
      usesTailwind: /className=["'][^"']*\b(?:bg-|text-|p-|m-|flex|grid)/.test(content),
...
function extractHardcodedSpacing(content) {
  const spacing = [];
  const spacingPattern = /(?:padding|margin|gap|space):\s*["']?\d+(?:px|rem|em)["']?/g;
...
function extractCustomColors(content) {
  ...
  const customPattern = /(?:bg-|text-|border-)(?!white|black|gray|red|blue|green|yellow|purple|pink|indigo)\w+/g;
...
function analyzeSeparationOfConcerns(content, filePath) {
  const relativePath = path.relative(SRC_DIR, filePath);
  const isComponent = relativePath.includes('components/') || relativePath.includes('pages/');
  ...
  return {
    ...
    businessLogic: {
      ...
      hasComplexCalculations: /\.reduce\(|\.filter\(.*\.map\(|for\s*\(.*for\s*\(/.test(content),
...
function extractServiceUsage(content) {
  const services = [];
  const servicePattern = /import\s+{[^}]*}\s+from\s+['"]@\/services\/([^'"]+)['"]/g;
  let match;
...
function detectSoCViolations(content) {
  ...
  if (responsibilities.length > 3) {
    violations.push(`Too many responsibilities: ${responsibilities.join(', ')}`);
  }
...
function extractTransitions(content) {
  const transitions = [];
  const transitionPattern = /transition-(?:all|colors|opacity|transform|[\w-]+)/g;
...
allFiles.forEach(filePath => {
  ...
  if (relativePath.includes('components/') || relativePath.includes('pages/')) {
    ...
    if (styling.tailwindClasses.length > 0 || styling.inconsistencies.length > 0) {
    ...
    if (animation.transitions.length > 0 || animation.animationClasses.length > 0) {
...
fs.writeFileSync(
  ...
  JSON.stringify({
    summary: {
      ...
      filesWithInlineLogic: authAnalysis.filter(a => a.hasInlineAuthLogic).length,
      uniqueRoles: [...new Set(authAnalysis.flatMap(a => a.roles))],
      uniquePermissions: [...new Set(authAnalysis.flatMap(a => a.permissions))],
    },
    files: authAnalysis,
  }, null, 2)
);
...
fs.writeFileSync(
  ...
  JSON.stringify({
    summary: {
      ...
      filesWithHardcodedColors: stylingAnalysis.filter(s => s.hardcodedColors.length > 0).length,
      filesWithInlineStyles: stylingAnalysis.filter(s => s.styling.usesInlineStyles).length,
      filesWithInconsistencies: stylingAnalysis.filter(s => s.inconsistencies.length > 0).length,
...
fs.writeFileSync(
  ...
  JSON.stringify({
    summary: {
      totalViolations: socAnalysis.reduce((sum, s) => sum + s.violations.length, 0),
  ...
  JSON.stringify({
    summary: {
      ...
      avgPropsPerComponent: Math.round(
        connectivityAnalysis.reduce((sum, c) => sum + c.dataFlow.propsReceived, 0) / connectivityAnalysis.length
      ),
...
fs.writeFileSync(
  ...
  JSON.stringify({
    summary: {
      ...
      filesWithTransitions: animationAnalysis.filter(a => a.animations.usesCSSTransitions).length,
      filesWithFramerMotion: animationAnalysis.filter(a => a.animations.usesFramerMotion).length,
...
fs.writeFileSync(
  ...
  JSON.stringify({
    summary: {
      filesWithIssues: performanceAnalysis.filter(p => p.issues.length > 0).length,
      filesWithMemoization: performanceAnalysis.filter(p => p.memoization.usesMemo > 0).length,
      filesWithLoading: performanceAnalysis.filter(p => p.loading.hasLoadingStates).length,
      filesWithLazyLoading: performanceAnalysis.filter(p => p.optimization.lazyLoading).length,
...
console.log(`  Files with styling issues: ${stylingAnalysis.filter(s => s.inconsistencies.length > 0).length}`);
...
console.log(`  Files with performance issues: ${performanceAnalysis.filter(p => p.issues.length > 0).length}\n`);
